...
{
    /* Setup ISR */
    BLE_MM01_uart_isr_StartEx(&CyBLE_Uart_Interrupt);

    BLE_MM01_HAL_Uart_Start();
}
...
void CyBLE_Uart_Stop (void)
{
    ...
    BLE_MM01_uart_isr_Stop();

    BLE_MM01_HAL_Uart_Stop();
}
...
{
    BLE_MM01_HAL_Uart_SpiUartPutArray(dataBuf, (uint32)length);
}
...
void CyBle_HalInit(void)
{
    ...
    BLE_MM01_bless_isr_SetVector(&CyBLE_Bless_Interrupt);
...
{
    BLE_MM01_bless_isr_StartEx(&CyBLE_Bless_Interrupt);
}
